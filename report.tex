...
water such as a lake. This wave may damage areas near or downstream of
the lake depending on its amplitude. Although it seems like a distant
and excessively rare phenomenon, thousands of people have been killed in
recent history (\autocite{Vajont}, \autocite{Palcacocha}) and even the
Léman's tsunami in the year 563 might have been caused by an impulse
wave.

Recent small-scale experiments have studied the effects of the
avalanche's characteristics on the wave's shape. We are interested in complementing
those recent studies by addressing the effect of topography.
...
heavy flows such as landslides and debris flows \autocite{heller}. This
relation has led to unrealistic predictions in engineering.

\includegraphics{media/s1.png} \includegraphics{media/s2.png}
\includegraphics{media/s3.png} Numerical simulation of an impulse wave
showing the free-surface elevation variation at times
\(t=1.1~\mathrm{s}\), \(t=2.2~\mathrm{s}\) and \(t=7.8~\mathrm{s}\).

Given this failure, an alternate method was proposed to estimate the
height of impulse waves, in the particular case of snow avalanches
\autocite{giboul}. It assumes a sudden transformation of the snow into
...
Trift dam \autocite{manso} to forecast the impulse waves and ensure they
will not jump over the dam and destroy the valley. It will serve as the
ground truth to our study.
...
numerical model \autocite{giboul} with varying slope, curvature
(vertically) and combe openness.
...
\(h_\mathrm{avalanche}/h_\mathrm{wave}^\mathrm{max}\) scales as
...
paths. To that end, we consider not a straight slope but one with
vertical curvature \(\varsigma\) and a bottleneck angle \(\beta\). They
are introduced as dimensionless variables and simply appended to the
product.
...
However, as avalanches should follow scale-invariance, a single
...
finding the coefficients of a widely different system :
...
\begin{quote}
\includesvg{media/bc.svg} Cross-sections of three avalanches (at the
boundary condition) with different bottleneck angles \(\beta\) but equal
areas and mean depths.
\end{quote}
...
\hypertarget{cd}{%
\centering
\includesvg{media/causal_diagram.svg}
\caption{Alt text}\label{cd}
}
\end{figure}
\end{quote}
